...
export default function UserManagement() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const mockUsers = [
        { id: 1, name: "Arjun Singh", email: "arjun@example.com", role: "admin", status: "Active", joined: "2024-01-15" },
        { id: 2, name: "Priya Sharma", email: "priya@shop.com", role: "shopkeeper", status: "Active", joined: "2024-02-10" },
        { id: 3, name: "Rahul Verma", email: "rahul@test.com", role: "user", status: "Active", joined: "2024-03-05" },
        { id: 4, name: "Sneha Gupta", email: "sneha@gmail.com", role: "user", status: "Inactive", joined: "2024-03-12" },
        { id: 5, name: "Vikram Malhotra", email: "vikram@business.com", role: "user", status: "Active", joined: "2024-01-20" },
      ];
      setUsers(mockUsers);
      setLoading(false);
    }, 1000);
  ...
  const handleRoleChange = async (userId, newRole) => {
    ...
    try {
      // API CALL EXAMPLE:
      // await fetch(`${import.meta.env.VITE_API}/api/users/${userId}/role`, {
      //   method: 'PUT',
  ...
  const getRoleIcon = (role) => {
    switch(role) {
      ...
      case 'shopkeeper': return <FaStore className="text-blue-500" />;
  ...
  const getRoleBadgeColor = (role) => {
    switch(role) {
      ...
      case 'shopkeeper': return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800';
        ...
        {loading ? (
            ...
            <>
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {filteredUsers.map((user) => (
                                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-full border text-xs font-medium ${getRoleBadgeColor(user.role)}`}>
                                            {getRoleIcon(user.role)}
                                            <span className="capitalize">{user.role}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${user.status === 'Active' ? 'text-green-700 bg-green-50 dark:bg-green-900/20 dark:text-green-400' : 'text-gray-700 bg-gray-100 dark:bg-gray-700 dark:text-gray-400'}`}>
                ...
                <div className="md:hidden grid grid-cols-1 gap-4 p-4">
                    {filteredUsers.map((user) => (
                        <div key={user.id} className="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${user.status === 'Active' ? 'text-green-700 bg-green-50 dark:bg-green-900/20 dark:text-green-400' : 'text-gray-700 bg-gray-100 dark:bg-gray-700 dark:text-gray-400'}`}>
                                    {user.status}
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-700 dark:text-gray-300 font-medium">{user.joined}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Current Role</p>
                                    <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-xs font-medium ${getRoleBadgeColor(user.role)}`}>
                                        {getRoleIcon(user.role)}
                                        <span className="capitalize">{user.role}</span>
                                    </div>
                                </div>
                            ...
                            <div className="pt-3 border-t border-gray-200 dark:border-gray-600 flex items-center justify-between">
                                <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Change Role:</span>
                                {updatingId === user.id ? (
